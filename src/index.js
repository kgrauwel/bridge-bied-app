const APP_VERSION = "2026-06-18-slam-evaluation-v1";
...
async function handleApi(request, env) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/new-session") {
      ...
      registry = await saveRegistry(env, registry);
      const newSession = initialSession(newSessionId, boardNumber, body.settings);
      newSession.members = {
        [seat]: playerName,
