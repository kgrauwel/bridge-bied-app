const APP_VERSION = "2026-06-18-admin-bbo-v1";
...
function scoreInfo(session, pairState) {
  ...
  const note = session.ddEnabled
    ? "DD analyse staat aan in admin, maar de externe DDS-solver is nog niet gekoppeld. Controleer deze hand voorlopig via de BBO-link."
    : "Richtscore: conservatief berekend uit punten, fit, controles en kwetsbaarheid. Echte par vereist double-dummy analyse.";
  return {
    par,
    final,
...
function normalizeAdminSettings(value) {
  ...
  return {
    ...
    ddEnabled: Boolean(source.ddEnabled),
  };
}
...
async function handleApi(request, env) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/new-session") {
      ...
      newSession.ddEnabled = Boolean(adminSettings.ddEnabled);
      newSession.members = {
        [seat]: playerName,
