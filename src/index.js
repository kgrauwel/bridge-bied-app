const APP_VERSION = "2026-06-16-player-sessions-v2";
...
function touchPlayers(registry, ...names) {
  ...
  for (const name of names.map(normalizePlayerName).filter(Boolean)) {
    registry.players[name] = { name, updatedAt: now };
  }
}
...
function publicState(session, pairLabel, seatValue, registry) {
  const pair = normalizePair(pairLabel);
...
async function handleApi(request, env) {
  ...
  try {
    ...
    if (request.method === "POST" && url.pathname === "/api/new-session") {
      ...
      const data = publicState(newSession, pair, seat, registry);
    ...
    if (request.method === "GET" && url.pathname === "/api/state") {
      ...
      const data = publicState(session, pair, seat, registry);
