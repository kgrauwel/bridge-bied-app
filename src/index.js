...
const APP_VERSION = "2026-06-19-dds-v11";
...
function naturalOpeningBid(stats, auction) {
  if (stats.hcp >= 22) return safeBid("2C", auction);
  if (stats.balanced && stats.hcp >= 20 && stats.hcp <= 21) return safeBid("2NT", auction);
...
function isStrongArtificialOpening(opening) {
  return opening?.call === "2C";
}

function responseToStrongOpening(stats, auction) {
  const major = bestMajor(stats, 5);
  if (major && stats.hcp >= 4) return lowestLegalBid(major, auction, 2) || "P";
  const long = longestSuit(stats);
  if (stats.hcp >= 5 && stats.distribution[long] >= 5) return lowestLegalBid(long, auction, 2) || "P";
  if (stats.balanced && stats.hcp >= 8) return safeBid("2NT", auction);
  return safeBid("2D", auction);
}
...
function responseToSlamAsk(hand, auction) {
  const aces = aceCount(hand);
  const response = aces === 0 || aces === 4 ? "5C" : aces === 1 ? "5D" : aces === 2 ? "5H" : "5S";
  return safeBid(response, auction);
}

function responseToOpening(seat, stats, auction, opening) {
  const [openLevel, openStrain] = bidParts(opening.call);
  if (isStrongArtificialOpening(opening)) return responseToStrongOpening(stats, auction);
...
function targetContractForSide(deal, side, ddTable = null) {
  return ddTable ? bestDoubleDummyContractForSide(deal, ddTable, side) : bestContractForSide(deal, side);
}

function sideHasCall(auction, side, call) {
  return auction.some((entry) => sideForSeat(entry.seat) === side && entry.call === call);
}

function strongOpenerRebid(seat, stats, auction, deal, ddTable = null) {
  const side = sideForSeat(seat);
  const target = targetContractForSide(deal, side, ddTable);
  if (target.bid && target.bid !== "Pas") {
    const [level] = bidParts(target.bid);
    if (level >= 6 && !sideHasCall(auction, side, "4NT")) {
      const slamAsk = safeBid("4NT", auction);
      if (slamAsk !== "P") return slamAsk;
    }
    if (ddTable && stats.hcp >= 20) return safeBid(target.bid, auction);
    if (level >= 6 && stats.hcp >= 23) return safeBid(target.bid, auction);
    if (level >= 4 && stats.hcp >= 22) return safeBid(target.bid, auction);
  }
  if (stats.balanced && stats.hcp >= 25) return safeBid("3NT", auction);
  if (stats.balanced && stats.hcp >= 22) return safeBid("2NT", auction);
  const major = bestMajor(stats, 5);
  if (major) return lowestLegalBid(major, auction, 2) || "P";
  return lowestLegalBid(longestSuit(stats), auction, 3) || "P";
}

function openerRebid(seat, stats, auction, opening, response, deal, ddTable = null) {
  if (isStrongArtificialOpening(opening)) return strongOpenerRebid(seat, stats, auction, deal, ddTable);
...
function finalChoiceForSide(deal, auction, side, ddTable = null) {
  ...
  const target = targetContractForSide(deal, side, ddTable);
  if (ddTable && target.bid && target.bid !== "Pas") return safeBid(target.bid, auction);
...
function simpleAutoBid(seat, hand, auction, partnership, deal, ddTable = null) {
  ...
  if (!ownBids.length && partnerLastBid?.call === "4NT") {
    return responseToSlamAsk(hand, auction);
  }
  ...
  if (ownBids.length === 1 && !partnerBids.length && isStrongArtificialOpening(ownBids[0])) {
    return strongOpenerRebid(seat, stats, auction, deal, ddTable);
  }
  ...
  if (ownBids.length === 1 && partnerBids.length === 1 && sideBids.length === 2 && sideBids[0].seat === seat) {
    return openerRebid(seat, stats, auction, ownBids[0], partnerBids[0], deal, ddTable);
  }

  if (ownBids.length >= 1 && partnerBids.length >= 1 && (sideBids.length >= 3 || sideHasCall(auction, sideForSeat(seat), "4NT"))) {
    return finalChoiceForSide(deal, auction, sideForSeat(seat), ddTable);
  }

  return "P";
}

async function processAutoOpponents(session, pairState) {
  let guard = 0;
  while (
    ...
    if (session.settings.opponentMode === "auto") {
      call = simpleAutoBid(seat, session.deal.hands[seat], pairState.auction, OPPONENT_SEATS, session.deal, null);
...
function removePushSubscription(registry, playerName, endpoint) {
  const name = normalizePlayerName(playerName);
  const cleanEndpoint = String(endpoint || "").trim();
  const player = registry.players[name];
  if (!name || !cleanEndpoint || !player?.pushSubscriptions) return false;
  const before = player.pushSubscriptions.length;
  player.pushSubscriptions = player.pushSubscriptions.filter((item) => item.endpoint !== cleanEndpoint);
  player.updatedAt = Date.now();
  return player.pushSubscriptions.length !== before;
}
...
async function notifyNextTurn(registry, session, pairState, actorSeat) {
  ...
  const actorPlayer = normalizePlayerName(session.members?.[actorSeat]);
  if (!targetPlayer || !actorPlayer || targetPlayer === actorPlayer) return false;
...
async function publicState(session, pairLabel, seatValue, registry, playerName = "") {
  ...
  await processAutoOpponents(session, pairState);
  const complete = isAuctionComplete(pairState.auction);
  const score = await scoreInfo(session, pairState);
  ...
  try {
    ...
    if (request.method === "GET" && url.pathname === "/api/sessions") {
      ...
      return json({
        ...
        settings: adminSettings.defaultGeneratorSettings,
      });
    }
    ...
    if (request.method === "POST" && url.pathname === "/api/push-unsubscribe") {
      const removed = removePushSubscription(registry, playerName, body.endpoint);
      registry = await saveRegistry(env, registry);
      return json({ removed });
    }
