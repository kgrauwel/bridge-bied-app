...
const APP_VERSION = "2026-06-19-dds-v10";
...
async function processAutoOpponents(session, pairState) {
  let guard = 0;
  let ddTable = null;
  if (session.settings.opponentMode === "auto" && session.ddEnabled !== false) {
    try {
  ...
  while (
    ...
    if (session.settings.opponentMode === "auto") {
      call = simpleAutoBid(seat, session.deal.hands[seat], pairState.auction, OPPONENT_SEATS, session.deal, ddTable);
